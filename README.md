...
| `@bot 人格切换 default` | 解除当前对话的人格绑定并恢复默认人格 |
| `@bot 人格列表` | 查看已保存人格 |
| `@bot 人格详情 <ID>` | 查看人格详情 |
...
| `人格切换 default` | 解除当前对话的人格绑定并恢复默认人格 |
| `人格列表 <群号>` | 查看指定群的人格 |
...
人格 ID 使用目标的 QQ 号，保证换群或修改群名片后仍能准确更新：
...
列表和详情仍会显示昵称；展示名称会带上群号、昵称和 QQ 号，例如：

```text
477065120-cbaba_2054716346
```
...
新生成的人格会尽量控制在少量核心字段内，避免画像过长导致回复像写作文：

| 字段 | 说明 |
|------|------|
| 身份设定 | 目标用户的圈层、核心气质、社交位置 |
| 人格核心 | 核心矛盾、情绪底色、典型反应 |
| 说话风格 | 句式、语气、长度、口癖 |
| 兴趣偏好 | 游戏、技术、硬件、音乐、消费等有证据的话题偏好 |
| 价值判断 | 公共议题、商业观、技术观、消费观等，不强行贴政治光谱标签 |
| 社交模式 | 对熟人/陌生人、接梗/反喷/锐评/求助时的表现 |
| 触发反应 | 遇到特定话题或场景时的反应 |
| 高频表达/爆点语录 | 常用短语和真正有记忆点的原话，宁缺毋滥 |
| 禁止项 | 生成回复时不该出现的风格偏差 |
...
- QQ 号是唯一身份键；同一用户换群或修改群名片后，默认更新原人格并刷新展示昵称。
- 不同用户即使群名片相同，只要 QQ 号不同，就会保存为不同人格。
- 如果加 `-f` 或 `--force`，会强制从零重建并覆盖旧人格。
- 旧版本因不同群名片生成的重复记录，会在下一次克隆该 QQ 时自动收拢，并保留旧 ID 作为兼容别名。
